Tarvarus Core version 0.18.1 is now available from:

  <https://download.tarvarus.org/tarvarus-0.18.1/>
...
`/Applications/Tarvarus-Qt` (on Mac) or `tarvarusd`/`tarvarus-qt` (on
Linux).
...
Tarvarus Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Tarvarus Core on unsupported systems.
...
10.10. Additionally, Tarvarus Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:17081:17081` (this is an extra `:17081` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@tarvarus.org) the Tarvarus Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 TVRS),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in TVRS per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/tarvarus-conf.md)
  about the `tarvarus.conf` file describes how to use it to configure
  Tarvarus Core.

- A new document introduces Tarvarus Core's BIP174 [Partially-Signed
  Tarvarus Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Tarvarus-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Tarvarus Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Tarvarus Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Tarvarus Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart tarvarusd with the `-deprecatedrpc=generate`
...
- A new `tarvarus-wallet` tool is now distributed alongside Tarvarus
...
This section describes planned changes to Tarvarus Core that may affect
other Tarvarus software and services.

- Since version 0.16.0, Tarvarus Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Tarvarus Core 0.20 (expected about
  a year after 0.18), Tarvarus Core will default to native segwit
  ...
  to bech32 addresses, and if the Tarvarus Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Tarvarus Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Tarvarus Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Tarvarus Core
...
- A sub-project of Tarvarus Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Tarvarus Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Tarvarus Core's own implementation, although entropy
  gathered by Tarvarus Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Tarvarus Core a
...
- On macOS, Tarvarus Core now opts out of application CPU throttling
...
- #17081 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
