...
The Tarvarus Core daemon can be configured to act as a trusted "border
router", implementing the tarvarus wire protocol and relay, making
...
The ZeroMQ feature in Tarvarus Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building tarvarusd:
...
For instance:
    ...
    $ tarvarusd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/tarvarusd.tx.raw \
...
These options can also be provided in tarvarus.conf.
...
From the perspective of tarvarusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tarvarusd directly. Furthermore, no information is
...
using. Tarvarusd appends an up-counting sequence number to each
